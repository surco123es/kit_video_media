...
#include "include/kit_video_media/kit_video_media_plugin.h"

#ifndef MEDIA_KIT_LIBS_NOT_FOUND

#include <gtk/gtk.h>

#include "include/kit_video_media/utils.h"
#include "include/kit_video_media/video_output_manager.h"

#define kit_video_media_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), kit_video_media_plugin_get_type(), \
                              KitVideoMediaPlugin))

struct _KitVideoMediaPlugin {
...
G_DEFINE_TYPE(KitVideoMediaPlugin, kit_video_media_plugin, g_object_get_type())

static void kit_video_media_plugin_handle_method_call(
    KitVideoMediaPlugin* self,
...
static void kit_video_media_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(kit_video_media_plugin_parent_class)->dispose(object);
}

static void kit_video_media_plugin_class_init(KitVideoMediaPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = kit_video_media_plugin_dispose;
}

static void kit_video_media_plugin_init(KitVideoMediaPlugin* self) {
  self->channel = NULL;
...
static void method_call_cb(FlMethodChannel* channel,
                           FlMethodCall* method_call,
                           gpointer user_data) {
  KitVideoMediaPlugin* plugin = kit_video_media_PLUGIN(user_data);
  kit_video_media_plugin_handle_method_call(plugin, method_call);
}

static KitVideoMediaPlugin* kit_video_media_plugin_new(
    FlPluginRegistrar* registrar) {
  KitVideoMediaPlugin* self = kit_video_media_PLUGIN(
      g_object_new(kit_video_media_plugin_get_type(), nullptr));
  g_autoptr(FlMethodCodec) codec =
      FL_METHOD_CODEC(fl_standard_method_codec_new());
  self->channel =
      fl_method_channel_new(fl_plugin_registrar_get_messenger(registrar),
                            "controller/kit_video_media", codec);
...
void kit_video_media_plugin_register_with_registrar(
    FlPluginRegistrar* registrar) {
  kit_video_media_plugin_new(registrar);
}
