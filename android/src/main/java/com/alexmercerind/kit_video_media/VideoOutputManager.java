...
public class VideoOutputManager {
    ...
    public VideoOutput create(long handle) {
        synchronized (lock) {
            Log.i("media_kit", String.format(Locale.ENGLISH, "com.alexmercerind.kit_video_media.VideoOutputManager.create: %d", handle));
    ...
    public void dispose(long handle) {
        synchronized (lock) {
            Log.i("media_kit", String.format(Locale.ENGLISH, "com.alexmercerind.kit_video_media.VideoOutputManager.dispose: %d", handle));
            if (videoOutputs.containsKey(handle)) {
    ...
    public long createSurface(long handle) {
        synchronized (lock) {
            Log.i("media_kit", String.format(Locale.ENGLISH, "com.alexmercerind.kit_video_media.VideoOutputManager.createSurface: %d", handle));
            if (videoOutputs.containsKey(handle)) {
    ...
    public void setSurfaceTextureSize(long handle, int width, int height) {
        synchronized (lock) {
            Log.i("media_kit", String.format(Locale.ENGLISH, "com.alexmercerind.kit_video_media.VideoOutputManager.setSurfaceTextureSize: %d %d %d", handle, width, height));
            if (videoOutputs.containsKey(handle)) {
