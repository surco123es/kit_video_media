...
void KitVideoMediaPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows* registrar) {
  auto plugin = std::make_unique<KitVideoMediaPlugin>(registrar);
  registrar->AddPlugin(std::move(plugin));
}

KitVideoMediaPlugin::KitVideoMediaPlugin(
    flutter::PluginRegistrarWindows* registrar)
    : registrar_(registrar),
      video_output_manager_(std::make_unique<VideoOutputManager>(registrar)) {
  channel_ = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
      registrar->messenger(), "controller/kit_video_media",
...
void KitVideoMediaPlugin::HandleMethodCall(
